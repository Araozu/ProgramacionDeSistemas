...
void ordenarFicheros(int numFichas, tipoDatos* ficheros) {

    tipoDatos ficheroAuxiliar;
    for (int i = 1; i < numFichas; i++) {
        tipoDatos ficheroAMover = ficheros[i];

        for (int j = i - 1; j >= 0; j--) {
            tipoDatos ficheroAComparar = ficheros[j];
            if (ficheroAMover.nombreFich.compare(ficheroAComparar.nombreFich) < 0) {
                ficheroAuxiliar.nombreFich = ficheroAMover.nombreFich;
                ficheroAuxiliar.tamanyo = ficheroAMover.tamanyo;

                ficheros[i].nombreFich = ficheroAComparar.nombreFich;
                ficheros[i].tamanyo = ficheroAComparar.tamanyo;

                ficheros[j].nombreFich = ficheroAuxiliar.nombreFich;
                ficheros[j].tamanyo = ficheroAuxiliar.tamanyo;
            } else {
                break;
            }
        }
